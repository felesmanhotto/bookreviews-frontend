...
export default function ReviewCard({ review }: { review: Review }) {
  ...
  return (
    <article className="rounded-2xl border border-slate-800 bg-sky-950 shadow-sm hover:shadow-md transition">
      <div className="flex items-stretch gap-5 p-4 md:p-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* review content */}
          <p className="mt-3 md:mt-4 text-sm md:text-base text-slate-200/90 line-clamp-4">
            {review.content}
          </p>
        </div>

        {/* user (placeholder) */}
        <aside className="hidden md:flex w-48 shrink-0 justify-end text-right">
          <div className="text-xs text-slate-400">
            {/* quando o feed trouxer dados do autor, renderize aqui */}
            <div>by <span className="font-medium text-slate-200">User #{review.user_id}</span></div>
            {/* botões/ações */}
          </div>
        </aside>
      </div>
